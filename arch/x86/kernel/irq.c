...
#else
#define inc_irq_stat_fast(index)	inc_irq_stat(index)
#endif
...
DEFINE_IDTENTRY_SYSVEC(sysvec_x86_platform_ipi)
{
	...
	inc_irq_stat_fast(X86_PLATFORM_IPI);
	if (x86_platform_ipi_callback)
...
DEFINE_IDTENTRY_SYSVEC(sysvec_posted_msi_notification)
{
	...
	inc_irq_stat_fast(POSTED_MSI_NOTIFICATION);
	irq_enter();
