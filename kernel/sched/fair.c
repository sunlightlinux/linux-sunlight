...
static inline void util_est_update(struct sched_entity *se)
{
	...
	/*
	 * util_est_update() is only called for task entities, from dequeue
	 * paths that already imply the task is going to sleep.
	 */
	trace_android_rvh_util_est_update(cfs_rq_of(se), task_of(se), true, &ret);
	if (ret)
		return;
