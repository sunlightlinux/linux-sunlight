...
#define EMA_ALPHA_VAL           96
#define EMA_ALPHA_SHIFT         7
#define MAX_RESCHED_INTERVAL_MS 100
#define LATENCY_MULTIPLIER      6

static DEFINE_PER_CPU(struct mobile_device, mobile_devices);

static inline u64 mobile_ema_new(u64 value, u64 ema_old)
{
        if (likely(ema_old))
...
static void mobile_reflect(struct cpuidle_device *dev, int index)
{
         ...
         */
        residency = div_u64(dev->last_residency_ns, NSEC_PER_USEC) - s->exit_latency;
        if (residency > 0)
                       ...
                       bool *stop_tick)
{
        ...
        for (i = 0; i < drv->state_count; i++) {
                ...
                if (s_flags & (CPUIDLE_FLAG_UNUSABLE | CPUIDLE_FLAG_OFF | CPUIDLE_FLAG_POLLING))
                        continue;

                if (s->power_usage != 0)
                        continue;

                if (!s->target_residency && i > 0)
                        continue;

                if (s->exit_latency > latency_req)
                        break;

                if (s->exit_latency * 5 > idle_duration_us * LATENCY_MULTIPLIER)
                        break;
